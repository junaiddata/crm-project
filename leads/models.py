...
# Same fields as the CRM models above, but stored in separate alabama_* tables
# in the same database so Alabama's data stays fully isolated from Junaid's.
# Served at /alabama/*; the views pick these models when the path is /alabama/.

class AlabamaWhatsAppLead(models.Model):
    MSG_TYPES = WhatsAppLead.MSG_TYPES
...
class AlabamaLead(models.Model):
    date             = models.DateField()
    mobile_no        = models.CharField(max_length=30, blank=True)
    email_id         = models.CharField(max_length=255, blank=True)
    name             = models.CharField(max_length=255, blank=True)
...
class AlabamaBroadcastJob(models.Model):
...
class AlabamaWhatsAppOutbound(models.Model):
    MSG_TYPES = WhatsAppOutbound.MSG_TYPES
